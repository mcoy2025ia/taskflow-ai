...
const GeneratePromptSchema = z.string().trim().min(1).max(4000).optional()

export async function generateTasksCSV(customPrompt?: string): Promise<
...
> {
  ...
  const parsedPrompt = GeneratePromptSchema.safeParse(customPrompt)
  if (!parsedPrompt.success) {
    return { success: false, error: parsedPrompt.error.issues[0]?.message ?? 'Prompt inválido' }
  }
  const prompt = parsedPrompt.data ?? DEFAULT_GENERATE_TASKS_PROMPT

  const apiKey = process.env.DEEPSEEK_API_KEY
  if (!apiKey) return { success: false, error: 'DEEPSEEK_API_KEY no configurada' }

  const baseUrl = process.env.AI_GATEWAY_BASE_URL ?? 'https://api.deepseek.com/v1'
  const model   = process.env.AI_GATEWAY_BASE_URL ? 'deepseek/deepseek-chat' : 'deepseek-chat'
